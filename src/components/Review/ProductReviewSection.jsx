  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      ...
      if (onDeleteReview) {
        ...
        try {
          if (!user)
            return alert("You need to be logged in to delete a review");
          const headers = {
            Authorization: `Bearer ${user.token}`,
          };
    ...
    <div className="mt-6 p-4 border rounded shadow">
      ...
      {loading ? (
        ...
        reviews.map((review) => (
          <div
            key={review.id}
            className="p-4 mb-2 border rounded shadow-sm bg-white"
          >
            <div className="flex justify-between items-center">
              ...
              {((user?.role === "user" &&
                (user.userId === review.user_id || user.id === review.user_id)) ||
